...
export class SalvageValueComponent {
  ...
  refreshData(){
    this.vehicleConfigService.getAllSalvageValue().subscribe({
      next: (res) => {
        if (res) 
          {
  ...
  saveData() {
    ...
    if (this.dataForm.valid) {
      if (this.dataForm.get("id")?.value) {
        ...
        this.vehicleConfigService.updateSalvageValue(newData).subscribe({
          next: (res) => {
            if (res.success) {
        ...
        const newData: SalvageValuePostDto = this.dataForm.value;
        this.vehicleConfigService.addSalvageValue(newData).subscribe({
          next: (res) => {
            if (res.success) {
              this.successAddMessage = res.message;
              this.closeModal();
