...
export class OrcStockComponent {
  ...
  refreshData() {
    ...
    this.stockManagementService.getAllOrcStock(pageNumber, pageSize, this.criteria, this.searchTerm).subscribe({
  ...
  getDatasforAddOrcStock() {
    this.vehicleConfig.getAllStockType().subscribe({
      next: (res) => {
        if (res) 
          {
            this.stocks = res
  ...
  saveData() {
    ...
    this.stockManagementService.addOrcStock(newData).subscribe({
  ...
  transferData(){
    ...
    if (this.dataForm1.valid) {
      ...
      this.stockManagementService.transferOrcStock({
